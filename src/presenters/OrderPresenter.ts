...
this.events.on(/^contacts\..*:change/, (data: { field: string; value: string }) => {
    if (['email', 'phone'].includes(data.field)) {
        this.appData.setContactField(data.field as keyof IContact, data.value);
    }
});
    ...
    private openOrderForm(): void {
        this.modalPresenter.openModal(this.orderForm.render({ address: '', errors: [], valid: false }));
    }

    private openContactsForm(): void {
        this.modalPresenter.openModal(this.contactsForm.render({ phone: '', email: '', errors: [], valid: false }));
    }

    private updateFormValidation(errors: Partial<IOrder>): void {
        const { address, email, phone } = errors;
        this.orderForm.valid = !address;
        this.orderForm.errors = Object.values({ address }).filter(Boolean).join('; ');
        this.contactsForm.valid = !email && !phone;
        this.contactsForm.errors = Object.values({ email, phone }).filter(Boolean).join('; ');
    }
    ...
    private submitContacts(): void {
        this.api.order(this.appData.getOrderToPost())
            .then((res) => {
